...
| `lookups.xlsx` | Thresholds and unit rates. Every `lkp_`-prefixed sheet is read and merged into one flat table addressed by (`lookup_set_name`, `setting_key`). This is where tunable numbers belong. **Treatment unit rates go in the `lkp_unit_rates` sheet specifically** — that is the one sheet the web app's Tuning → Treatment Rates tab reads, by name. |
